...
def verifyUnpacked(java, artifact, unpackPath, gitRevision, version, testArgs):
  ...
  if isSrc:
    ...
    print("Alt javas are",java.run_alt_javas, java.alt_java_versions)
    ...
    if java.run_alt_javas:
      for run_alt_java, alt_java_version in zip(java.run_alt_javas, java.alt_java_versions):
        print("The alt java version", alt_java_version)
        print("    run tests w/ Java %s and testArgs='%s'..." % (alt_java_version, testArgs))
        run_alt_java('./gradlew --no-daemon clean test %s' % testArgs, '%s/test-java%s.log' % (alt_java_version, unpackPath))
        print("    run integration tests w/ Java %s" % alt_java_version)
        run_alt_java('./gradlew --no-daemon integrationTest -Dversion.release=%s' % version, '%s/itest-java%s.log' % (alt_java_version, unpackPath))
        print("    build binary release w/ Java %s" % alt_java_version)
        run_alt_java('./gradlew --no-daemon dev -Dversion.release=%s' % version, '%s/assemble-java%s.log' % (alt_java_version, unpackPath))
        testSolrExample("%s/solr/packaging/build/dev" % unpackPath, alt_java_version)
    ...
    print("Java Base Version", javaBaseVersionUnpackPath, "Java Home", java.java_home)
    ...
    if java.run_alt_javas:
      for run_alt_java, alt_java_version in zip(java.run_alt_javas, java.alt_java_versions):
        ...
        testSolrExample(javaAltVersionUnpackPath, alt_java_version)
...
def parse_config():
  ...
  if c.version is not None:
    ...
    print("The version is", c.version)
  ...
  print("test alt java is",c.test_alt_java)
